...
- Stale examples → code should match current API; verify method names against the inventory

**Critical:** Always verify that method/function names in code examples actually exist in the code inventory. Outdated method names are a common failure mode in tutorials.
...
[For docs with code examples, compare method/function calls against the code inventory:
- Extract all method names, function calls, and class names from code blocks
- Check each against the inventory - does this method/function actually exist?
- Flag any mismatches: wrong method names, outdated APIs, incorrect signatures
- If something looks suspicious, use Grep to check the actual source code]

**Mismatches found:** [List any methods in docs that don't match the inventory, or None]
...
- [ ] Code examples use correct API (methods/functions exist and match inventory)
